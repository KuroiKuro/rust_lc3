...
fn add_op(instr: u16, vm: &mut Lc3Vm) {
    ...
    let first_val: i16 = vm.get_reg_val_by_id(first_reg).try_into().unwrap();
    ...
    let second_val: i16 = if register_mode {
        ...
        vm.get_reg_val_by_id(second_reg).try_into().unwrap()
    } else {
    ...
    vm.set_reg_val_by_id(dest_reg, added.try_into().unwrap());
